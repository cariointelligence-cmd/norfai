...
function hostFromUrlOrHost(raw: string | undefined): string | undefined {
...
const SKIP_HOST =
  /(^|\.)(x\.ai|stripe\.com|googleapis\.com|github\.com|neon\.tech|auth\.grok\.me)$/i;

function collectDeployHosts(): string[] {
...
function ownOriginFromRequest(request?: Request): string | undefined {
...
const deployHosts = collectDeployHosts();
const deployOrigins = deployHosts.map(originFromHost);

// Always resolve baseURL from the inbound Host so a custom domain (and grok.me)
// each mint OAuth callbacks on the host the visitor actually used. A static
// BETTER_AUTH_URL pinned to grok.me is what bounced Google login back to the
// old hostname after a custom domain was attached.
const baseURL = {
  allowedHosts: [
    ...previewAllowedHosts,
    "*.grok.me",
...
// Missing entries here surface as FORBIDDEN "Invalid origin". Include the
// request's own origin so a mapped custom domain is trusted even when it was
// not present in BETTER_AUTH_URL at build time. Do not wildcard "*" — that
// would also trust attacker sites.
const trustedOrigins = (request?: Request) => {
  const own = ownOriginFromRequest(request);
  const headerOrigin = request?.headers.get("origin") ?? undefined;
  const extra: string[] = [];
  if (own) extra.push(own);
  if (headerOrigin && own && headerOrigin === own) extra.push(headerOrigin);
  return [
    ...previewAllowedHosts,
...
/** Remember the inbound Host so OAuth redirect_uri stays on a custom domain. */
export function registerInboundAuthHost(request: Request): void {
  const own = ownOriginFromRequest(request);
  if (!own) return;
  let host: string;
  try {
    host = new URL(own).hostname.toLowerCase();
  } catch {
    return;
  }
