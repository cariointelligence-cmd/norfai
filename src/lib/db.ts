...
if (typeof window === "undefined") {
  ...
  const ready = globalBoot.__pgBootstrapPromise__ ?? Promise.resolve();
  void ready.then(() => {
    void import("./norr/worker.ts").then((m) => m.ensureWorker()).catch((err) => {
      console.error("[norf-worker] start failed", err);
    });
  });
}
